...
export default function useCoffeeCalculator(): State {
  const [coffeeWeight, setCoffeeWeight] = useLocalStorage<string>(
    LocalStorageKey.CoffeeWeight,
    ''
  );

  const calculateWaterWeight = useCallback(
    (newCoffeeWeight: string): string => {
  ...
  const handleCoffeeWeightChange = useCallback(
